...
テストの簡素化やメモリの不可軽減等を実現するため、Lightningメッセージの長さは最大で65535バイトと定められています。  
  ...
  遡及をより困難にするため、Lightningメッセージの先頭に付加される文字列の長さも暗号化されます。  
さらに、16バイトのMAC(Poly-1305タグ)が、前述のLightningメッセージの先頭に付加される文字列の長さが通信の途中で改竄されていないことを確認するために、前述の暗号化された文字列に付加されます。

#### メッセージの全体  

2-byte encrypted message length  
16-byte MAC of the encrypted message length  
encrypted Lightning message(MAX:65535-byte)  
16-byte MAC of the Lightning message  

#### 暗号化とメッセージの送信

以下の工程によってLightningメッセージ(m)を送信するために、送信鍵(sk)とナンス(sn)が与えられます。
...
ナンス('sn')は96ビットのリトルエンディアンでエンコーxドされます。  
